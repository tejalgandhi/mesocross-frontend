...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getPaymentObject (orderPayload) {
                const paymentMethod = this.getSelectedMethod()
                const temporaryPaymentObject = {
                    payment_method: paymentMethod,
                    amount: Number(this.selectedPlan.price) * 100, // here the amount will be in the smallest unit of currency.
                    currency: 'eur'
                }
            ...
            continueCheckout () {
                if (this.tabIndex === 2) { // PLACE AN ORDER
                    ...
                    this.frontPayment.pay(this.getPaymentObject(orderPayload))
                        .then((paymentResponse) => {
                            console.log('payment Response', paymentResponse)
                            orderPayload.payment_response = paymentResponse
                            this.$axios.post('/subscription/plan/store', orderPayload).then(({ data }) => {
                                ...
                                console.log('Error while save subscription: ', err)
                            })
                        })
                        .catch((err) => {
